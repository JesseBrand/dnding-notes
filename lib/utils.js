...
export function getSinglePost (slug, title) {
  ...
  const [htmlContent] = Transformer.getHtmlContent(fileContent, title)
  // console.log("==================================")
...
const cachedTree = convertObject(getDirectoryData())
...
export function toSlug (filePath) {
  if (Node.fileExists(filePath) && filePath.includes(Node.getMarkdownFolder())) {
...
export function convertObject (thisObject) {
  ...
  const newObject = {
    ...
    label: thisObject.name.replace('.md', ''),
    children,
...
export function commaAndList (aList) {
  switch (aList.length) {
    case 0:
      return ''
    case 1:
      return aList[0]
    default:
      return aList.slice(0, aList.length - 1).join(', ') + ' and ' + aList[aList.length - 1]
  }
}
